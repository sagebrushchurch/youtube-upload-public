...
def upload_youtube_video(youtube, options, video_path, total_videos, index):
    ...
    try:
        video_id = upload_video.upload(youtube, video_path, request_body, progress_callback=progress.callback, chunksize=options.chunksize)
    finally:
        progress.finish()
    return video_id
...
def get_youtube_handler(options):
    ...
    default_credentials = os.path.join(home, "youtube-upload-master/bin/my-credentials.json")
    client_secrets = options.client_secrets or os.path.join(home, "youtube-upload-master/bin/client_secrets.json")
...
def main(arguments):
    ...
    # Additional options
    parser.add_option('', '--chunksize', dest='chunksize', type="int", default=1024 * 1024 * 8, help='Update file chunksize')
    parser.add_option('', '--open-link', dest='open_link', action='store_true', help='Opens a url in a web browser to display the uploaded video')
